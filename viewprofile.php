<?php
  ...
  $page_title = 'View Profile of Members';
  ...
  if (!isset($_GET['user_id'])) {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture, email, phone FROM admin WHERE user_id = '" . $_SESSION['user_id'] . "'";
  }
  else {
    $query = "SELECT username, first_name, last_name, gender, birthdate, city, state, picture, email, phone FROM admin WHERE user_id = '" . $_GET['user_id'] . "'";
  }
    ...
    if (!empty($row['picture'])) {
	 ...
	 if (!empty($row['email'])) {
      echo '<tr><td class="label">Email:</td><td>' . $row['email'] . '</td></tr>';
		 ...
		 if (!empty($row['phone'])) {
      echo '<tr><td class="label">Phone no:</td><td>' . $row['phone'] . '</td></tr>';
    }
	  echo '</table>';
    if (!isset($_GET['user_id']) || ($_SESSION['user_id'] == $_GET['user_id'])) {
